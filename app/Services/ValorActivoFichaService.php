    ...
    public function xlsxPayload(object $record): array
    {
        return [
            ...
            'rows' => [[
                ...
                $this->statusLabel($record->metodo_depreciacion),
                $record->fecha_inicio_depreciacion,
                $this->decimalOrNull($record->valor_residual),
                $this->decimalOrNull($record->depreciacion_estimada),
                $this->decimalOrNull($record->valor_en_libros_estimado),
                $record->calculo_depreciacion_at,
                $record->fecha_corte,
        ...
        return [
            ...
            'rows' => [
                ...
                ['Valores', 'Vida útil', $record->vida_util_meses !== null
                    ...
                    : 'Sin definir'],
                ['Depreciación referencial', 'Método', $this->display($this->statusLabel($record->metodo_depreciacion))],
                ['Depreciación referencial', 'Fecha de inicio', $this->displayDate($record->fecha_inicio_depreciacion)],
                ['Depreciación referencial', 'Valor residual', $this->money($record->valor_residual, $moneyCurrency)],
                ['Depreciación referencial', 'Depreciación estimada', $this->money($record->depreciacion_estimada, $moneyCurrency)],
                ['Depreciación referencial', 'Valor en libros estimado', $this->money($record->valor_en_libros_estimado, $moneyCurrency)],
                ['Depreciación referencial', 'Fecha de cálculo', $this->displayDateTime($record->calculo_depreciacion_at)],
                ...
                ['Estado técnico del XML', 'Resultado', $this->display($this->statusLabel($record->conciliacion_cfdi))],
                ['Estado técnico del XML', 'Validación técnica', $this->display($this->statusLabel($record->cfdi_estatus))],
                ['Estado técnico del XML', 'UUID extraído', $this->display($record->cfdi_uuid)],
                ['Estado técnico del XML', 'Total extraído', $this->money($record->cfdi_total, (string) ($record->cfdi_moneda ?: $moneyCurrency))],
                ['Estado técnico del XML', 'Moneda extraída', $this->display($record->cfdi_moneda)],
